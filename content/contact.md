---
title: "Contact"
description: "Neem contact op met Zweefvliegclub Flevo."
has_map: true
menu:
  main:
    weight: 5
---

## Neem contact op

Heb je vragen over zweefvliegen, een introductievlucht, het lidmaatschap of iets anders? We horen graag van je.

<div class="contact-grid">
<div class="contact-info">

<div class="contact-detail">
  <div class="contact-detail-icon">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
  </div>
  <div class="contact-detail-text">
    <strong>Adres</strong>
    <p>{{< adres "block" >}}</p>
  </div>
</div>

<div class="contact-detail">
  <div class="contact-detail-icon">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
  </div>
  <div class="contact-detail-text">
    <strong>Secretariaat</strong>
    <p>{{< email >}}</p>
  </div>
</div>

<div class="contact-detail">
  <div class="contact-detail-icon">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.63A2 2 0 012 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>
  </div>
  <div class="contact-detail-text">
    <strong>Telefoon (bij aankomst)</strong>
    <p>{{< veldtelefoon >}}</p>
  </div>
</div>

<div class="contact-detail">
  <div class="contact-detail-icon">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
  </div>
  <div class="contact-detail-text">
    <strong>Zomercursus</strong>
    <p><a href="mailto:zomercursus@zcflevo.nl">zomercursus@zcflevo.nl</a><br>Hans van Zandvoort</p>
  </div>
</div>

<div class="contact-detail">
  <div class="contact-detail-icon">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.63A2 2 0 012 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>
  </div>
  <div class="contact-detail-text">
    <strong>PR &amp; Media</strong>
    <p><a href="mailto:pr@zcflevo.nl">pr@zcflevo.nl</a><br>Tom Uneken, <a href="tel:0642013744">06-42 01 37 44</a></p>
  </div>
</div>

</div>
<div>

{{< mapbox lng="5.937581961427093" lat="52.06107763169907" zoom="10" >}}

<div class="notice-box">
  <strong>Belangrijk:</strong> Maak voor een bezoek altijd eerst een afspraak per mail. Betreed het vliegveld niet zelfstandig. Bij aankomst op de parkeerplaats bel je {{< veldtelefoon >}}, dan halen we je op en begeleiden je veilig naar de startplaats.
</div>

</div>
</div>
